...
You'll perform almost all animations on [a](/docs/vue-motion-component.md) \
...
Motion offers [the](/docs/vue-motion-component#whileinview.md) \
...
Usually, these are created automatically by \
...
Usually, dragging is initiated by pressing down on [a](/docs/vue-gestures#drag.md) \
...
In this guide, we'll learn how to use \
...
Motion makes it simple to **animate an element's size and position** between different layouts. With the \
...
Motion offers [the](/docs/react-motion-component#whileinview.md) \

---

# SVG animation

Motion makes React SVG animation straightforward. In this guide, we'll learn how to make line drawing animations, path morphing animations, animate \
...
Most [React animations](/docs/react-animation.md) in Motion are powered by the \
...
To link Motion 3D context with DOM Motion, for example to share a default transition or link the [LayoutCamera](/docs/react-three-fiber-layout-cameras.md) with layout animations, the \
...
Before you install Motion, note that Motion is only compatible with React versions \
...
Usually, dragging is initiated by pressing down on [a](/docs/react-gestures#drag.md) \
...
Motion for React provides APIs that make it simple to respect these people's preferences. In this guide, we'll learn how to use the \
...
The best part? It's also tiny, with a mini HTML/SVG version of the \
...
Motion Values are usually created automatically by the \
...
When animating elements with the [animate](/docs/animate.md) function, it's possible to stagger animations across them using \
...
### 1\\. Your browser doesn't support \
...
You might already know that for best animation performance you should stick to animating \

---

# Improvements to Web Animations API

Motion is the only animation library with a hybrid engine, meaning its capable of dynamically running animations either via \
...
Motion is built on modern browser APIs like [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API) (WAAPI) and [Scroll Timeline](https://developer.mozilla.org/en-US/docs/Web/API/ScrollTimeline), which is what enables it to offer hardware acceleration for common animations like \
...
Motion 12 introduces a small change to the \

---

# glide

**Note:** This API is **deprecated**. You can use \
